<template>
  <svg version="1.1" :class="clazz" :role="label ? 'img' : 'presentation'" :aria-label="label" :width="width"
    ...
    <path :opacity="path && path.opacity" :transform="path && path.transform" :d="path && path.d" :fill="path && path.fill" :stroke="path && path.stroke" v-for="path in icon.paths"/>
  </svg>
</template>
